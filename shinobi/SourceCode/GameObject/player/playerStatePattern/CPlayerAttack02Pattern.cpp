...
//コンストラクタ
CPlayerAttack02Pattern::CPlayerAttack02Pattern(CPlayer* pPlayer , CPlayerPatternManager* pManager)
{
	//武器によるモーション初期化
	if (pPlayer != nullptr) {
		//武器によるニュートラルモーション設定
		...
		case CPlayer::WEAPON_WSWORD:
			pPlayer->SetMotionMode(CPlayer::MOTION_WSWORD_ATTACK_TWO);
			m_nMotionTime = pPlayer->GetMotionTotalFrame((int)CPlayer::MOTION_WSWORD_ATTACK_TWO);
			pPlayer->SetAttackType(WSWORD_ATTACK_02);
			break;
		case CPlayer::WEAPON_CLAYMORE:
			pPlayer->SetMotionMode(CPlayer::MOTION_CLAYMORE_ATTACK_TWO);
			m_nMotionTime = pPlayer->GetMotionTotalFrame((int)CPlayer::MOTION_CLAYMORE_ATTACK_TWO);
			pPlayer->SetAttackType(CLAYMORE_ATTACK_02);
			break;
		}

		m_MotionNumOld = pPlayer->GetMotionNow();
		pPlayer->SetStateNow(PLAYER_ATTACK02_PATTERN);
	}
...
//デストラクタ
CPlayerAttack02Pattern::~CPlayerAttack02Pattern()
{
	m_LeftWeaponATKEffect.Uninit();
	m_RightWeaponATKEffect.Uninit();
}

bool CPlayerAttack02Pattern::InitParameter(CPlayer* pPlayer , CPlayerPatternManager* pManager)
{
	//武器によるモーション初期化
	if (pPlayer != nullptr) {
		//武器によるニュートラルモーション設定
...
void CPlayerAttack02Pattern::Update(CPlayer* pPlayer)
{
	//ヌルチェック
	if (pPlayer == nullptr) {
		return;
...
//描画処理
void CPlayerAttack02Pattern::Draw(void)
{
	m_LeftWeaponATKEffect.Draw();
	m_RightWeaponATKEffect.Draw();
}

void CPlayerAttack02Pattern::UpdateAttackEffectPosition(CPlayer* pPlayer)
{
	//ヌルチェック
	if (pPlayer == nullptr) {
		return;
...
void CPlayerAttack02Pattern::UpdateWSwordAttackEffect(CPlayer* pPlayer)
{
	//ヌルチェック
	if (pPlayer == nullptr) {
		return;
...
void CPlayerAttack02Pattern::UpdateClaymoreAttackEffect(CPlayer* pPlayer)
{
	//ヌルチェック
	if (pPlayer == nullptr) {
		return;
...
//武器のエフェクト停止
void CPlayerAttack02Pattern::StopWeaponEffect(void)
{
	m_LeftWeaponATKEffect.NoUseAllVertex();
	m_RightWeaponATKEffect.NoUseAllVertex();
}
...
//武器ごとの攻撃モーションの具体動作
void CPlayerAttack02Pattern::AttackActionDetail(CPlayer* pPlayer)
{
	//ヌルチェック
	if (pPlayer == nullptr) {
		return;
	...
	case CPlayer::WEAPON_WSWORD:
		...
		if (HitColliEnable) {
			pPlayer->EnableWeaponHitCircle(true, false);
		}
		else {
	...
	case CPlayer::WEAPON_CLAYMORE:
		...
		//当り判定円の有効無効設定部分
		HitColliEnable = pPlayer->GetKeyFrameNow() != 0;
		if (HitColliEnable) {
		...
		//攻撃力設定
		pPlayer->SetAttackPower(60);
		break;
	}
}

//パターンの変更
bool CPlayerAttack02Pattern::ChangePattern(CPlayer* pPlayer)
{
	//ヌルチェック
	if (pPlayer == nullptr) {
		return false;
	...
	if ((MotionTotalKeyFrame - 1) == KeyFrameNow) {
		...
		case CPlayer::WEAPON_WSWORD:
		{
			CPlayerPattern* pPlayerPattern = m_PlayerPatternManager->GetPattern(PLAYER_ATTACK03_PATTERN);
			if (pPlayerPattern != nullptr) {
				pPlayerPattern->InitParameter(pPlayer, m_PlayerPatternManager);
			}
			pPlayer->DisableWeaponHitCircle(true, true);
...
void CPlayerAttack02Pattern::ChangeDirWhenAttack(CPlayer* pPlayer)
{
	//ヌルチェック
	if (pPlayer == nullptr) {
		return;
...
D3DXVECTOR2 CPlayerAttack02Pattern::KeyboardValue(void)
{
