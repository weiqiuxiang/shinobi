...
//コンストラクタ
CPlayerAttack03Pattern::CPlayerAttack03Pattern(CPlayer* pPlayer, CPlayerPatternManager* pManager)
{
	//武器によるモーション初期化
	if (pPlayer != nullptr) {
		//武器によるニュートラルモーション設定
		...
		case CPlayer::WEAPON_WSWORD:
			pPlayer->SetMotionMode(CPlayer::MOTION_WSWORD_ATTACK_THREE);
			m_nMotionTime = pPlayer->GetMotionTotalFrame((int)CPlayer::MOTION_WSWORD_ATTACK_THREE);
			pPlayer->SetAttackType(WSWORD_ATTACK_03);
			break;
		case CPlayer::WEAPON_CLAYMORE:
			pPlayer->SetMotionMode(CPlayer::MOTION_CLAYMORE_ATTACK_THREE);
			m_nMotionTime = pPlayer->GetMotionTotalFrame((int)CPlayer::MOTION_CLAYMORE_ATTACK_THREE);
			pPlayer->SetAttackType(CLAYMORE_ATTACK_03);
			break;
		}

		m_MotionNumOld = pPlayer->GetMotionNow();
		pPlayer->SetStateNow(PLAYER_ATTACK03_PATTERN);
	}
...
//デストラクタ
CPlayerAttack03Pattern::~CPlayerAttack03Pattern()
{
	m_LeftWeaponATKEffect.Uninit();
	m_RightWeaponATKEffect.Uninit();
}

bool CPlayerAttack03Pattern::InitParameter(CPlayer* pPlayer , CPlayerPatternManager* pManager)
{
	//武器によるモーション初期化
	if (pPlayer != nullptr) {
		//武器によるニュートラルモーション設定
...
void CPlayerAttack03Pattern::Update(CPlayer* pPlayer)
{
	//ヌルチェック
	if (pPlayer == nullptr) {
		return;
...
//描画処理
void CPlayerAttack03Pattern::Draw(void)
{
	m_LeftWeaponATKEffect.Draw();
	m_RightWeaponATKEffect.Draw();
}

void CPlayerAttack03Pattern::UpdateAttackEffectPosition(CPlayer* pPlayer)
{
	//ヌルチェック
	if (pPlayer == nullptr) {
		return;
...
void CPlayerAttack03Pattern::UpdateWSwordAttackEffect(CPlayer* pPlayer)
{
	//ヌルチェック
	if (pPlayer == nullptr) {
		return;
...
void CPlayerAttack03Pattern::UpdateClaymoreAttackEffect(CPlayer* pPlayer)
{
	//ヌルチェック
	if (pPlayer == nullptr) {
		return;
...
//武器のエフェクト停止
void CPlayerAttack03Pattern::StopWeaponEffect(void)
{
	m_LeftWeaponATKEffect.NoUseAllVertex();
	m_RightWeaponATKEffect.NoUseAllVertex();
}
...
//武器ごとの攻撃モーションの具体動作
void CPlayerAttack03Pattern::AttackActionDetail(CPlayer* pPlayer)
{
	//ヌルチェック
	if (pPlayer == nullptr) {
		return;
	...
	case CPlayer::WEAPON_CLAYMORE:
		...
		//当り判定円の有効無効設定部分
		HitColliEnable = pPlayer->GetKeyFrameNow() != 0 && pPlayer->GetKeyFrameNow() != 1;
		if (HitColliEnable) {
			pPlayer->EnableWeaponHitCircle(true, true);
		}
		else {
		...
		//攻撃力設定
		pPlayer->SetAttackPower(80);
		break;
	}
}

//パターンの変更
bool CPlayerAttack03Pattern::ChangePattern(CPlayer* pPlayer)
{
	//ヌルチェック
	if (pPlayer == nullptr) {
		return false;
	...
	if ((MotionTotalKeyFrame - 1) == KeyFrameNow) {
		...
		case CPlayer::WEAPON_WSWORD:
		{
			CPlayerPattern* pPlayerPattern = m_PlayerPatternManager->GetPattern(PLAYER_ATTACK04_PATTERN);
			if (pPlayerPattern != nullptr) {
				pPlayerPattern->InitParameter(pPlayer, m_PlayerPatternManager);
			}
			pPlayer->DisableWeaponHitCircle(true, true);
...
void CPlayerAttack03Pattern::ChangeDirWhenAttack(CPlayer* pPlayer)
{
	//ヌルチェック
	if (pPlayer == nullptr) {
		return;
...
D3DXVECTOR2 CPlayerAttack03Pattern::KeyboardValue(void)
{
