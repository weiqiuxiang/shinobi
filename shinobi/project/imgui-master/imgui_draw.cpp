...
void ImDrawList::UpdateTextureID()
{
    ...
    // Try to merge with previous command if it matches, else use current command
    ImDrawCmd* prev_cmd = CmdBuffer.Size > 1 ? curr_cmd - 1 : nullptr;
    if (prev_cmd && prev_cmd->TextureId == curr_texture_id && memcmp(&prev_cmd->ClipRect, &GetCurrentClipRect(), sizeof(ImVec4)) == 0 && prev_cmd->UserCallback == nullptr)
        CmdBuffer.pop_back();
    else
...
bool    ImFontAtlasBuildWithStbTruetype(ImFontAtlas* atlas)
{
    ...
    stbtt_PackBegin(&spc, nullptr, atlas->TexWidth, max_tex_height, 0, atlas->TexGlyphPadding, nullptr);
    ...
    for (int input_i = 0; input_i < atlas->ConfigData.Size; input_i++)
    {
        ...
        dst_font->FallbackGlyph = nullptr; // Always clear fallback so FindGlyph can return nullptr. It will be set again in BuildLookupTable()
