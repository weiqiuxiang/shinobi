...
{
	//SE
	{TITLE_SE_CURSOR_PASS, 0},
	{TITLE_SE_DECITION_PASS, 0 },
...
HRESULT SetSoundVolume(SOUND_LABEL label, float fVolume)
{
	XAUDIO2_VOICE_STATE xa2state;

	//ボリュームの値制限
	fVolume = max(0.0f, fVolume);         //0.0f以上制限
	fVolume = min(fVolume,1.0f);          //1.0f以下制限

	// 状態取得
	g_apSourceVoice[label]->GetState(&xa2state);
	if (xa2state.BuffersQueued != 0)
	{// 再生中
		g_apSourceVoice[label]->SetVolume(fVolume);     //ボリューム設定
	}
...
void SetAllSoundVolum(float fVolume)
{
	...
	for (int nCntSound = 0; nCntSound < SOUND_LABEL_MAX; nCntSound++) {
		...
		if (xa2state.BuffersQueued != 0)
		{// 再生中
			g_apSourceVoice[nCntSound]->SetVolume(fVolume);     //ボリューム設定
		}
	}
}

HRESULT GetSoundVolume(SOUND_LABEL label, float *pVolume)
{
	XAUDIO2_VOICE_STATE xa2state;
	...
	if (xa2state.BuffersQueued != 0)
	{// 再生中
		g_apSourceVoice[label]->GetVolume(pVolume);     //ボリューム設定
	}
